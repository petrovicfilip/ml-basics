...
# ============================================================================
# DODATO: pun deletion po Pokrajcu (2007), sekcija o brisanju tacaka.
# Brisanje tacke p utice samo na njene reverzne susede RkNN(p): njima se menja
# susedstvo (p ispada), pa im treba preracunati k_dist -> lrd -> lof. Lancano se
# azuriraju i RkNN tih tacaka (jer lof zavisi od lrd suseda).
# ============================================================================
def reindex_after_removal(removed, x_list, neighborhoods, rev_neighborhoods,
                          k_dist, reach_dist, dist_dict, local_reach, lof):
    """Fizicki ukloni indeks `removed` i pomeri sve vece indekse za -1.

    Posto je ceo algoritam pozicioni (tacka = x_list[i]), posle brisanja moramo
    da reindeksiramo SVE strukture da indeksi ostanu uzastopni 0..n-1.
    """
    def shift(idx):
        # nov indeks posle uklanjanja `removed`
    ...
    # 2) pomocna za remapiranje dict-a indeksiranog celim brojem
    def remap_int_dict(d, default_factory=None):
        nd = {}
        for k, v in d.items():
            if k == removed:
                continue
    ...
    # 3) pomocna za remapiranje liste indeksa (izbaci removed, pomeri ostale)
    def remap_index_list(lst):
        return [shift(i) for i in lst if i != removed]

    # neighborhoods / rev_neighborhoods: i kljucevi i vrednosti su indeksi
    neighborhoods = {shift(k): remap_index_list(v)
                     for k, v in neighborhoods.items() if k != removed}
    rev_neighborhoods = {shift(k): remap_index_list(v)
                         for k, v in rev_neighborhoods.items() if k != removed}

    # k_dist / local_reach / lof: kljuc je indeks, vrednost skalar/lista
    k_dist = remap_int_dict(k_dist)
    local_reach = remap_int_dict(local_reach)
    lof = remap_int_dict(lof)

    # dist_dict / reach_dist: dict[i] -> dict[j] -> vrednost; i i j su indeksi
    def remap_nested(d):
        nd = {}
    ...
    dist_dict = remap_nested(dist_dict)
    reach_dist = remap_nested(reach_dist)

    return (x_list, neighborhoods, rev_neighborhoods, k_dist,
            reach_dist, dist_dict, local_reach, lof)
...
class LocalOutlierFactor(anomaly.base.AnomalyDetector):
    ...
    def __init__(
        ...
        window_size: int | None = None,  # DODATO: max tacaka; None = neograniceno (original)
        min_dist: float = 0.0,           # DODATO: prag za near-duplikate (0 = iskljuceno)
    ):
        self.n_neighbors = n_neighbors
        self.window_size = window_size   # DODATO
        self.min_dist = min_dist         # DODATO
        self.x_list: list = []
        self.x_batch: list = []
        self.x_scores: list = []
        self.dist_dict: dict = {}
    ...
    def learn_one(self, x: dict):
        # DODATO: near-duplicate filter -> preskoci tacku prakticno identicnu
        # nekoj od poslednjih u prozoru (ne trosi mesto na redundantni saobracaj)
        if self.min_dist > 0 and self.x_list:
            for old in self.x_list[-50:]:
                if self.distance(x, old) <= self.min_dist:
                    return self
        ...
        # DODATO: sliding window -> ako prozor pun, obrisi najstariju (poziciju 0)
        # punim Pokrajac deletion-om. Memorija ima plafon = window_size.
        if self.window_size is not None:
            while len(self.x_list) > self.window_size:
                self.delete_one(0)
        return self
    ...
    def delete_one(self, index: int):
        """DODATO: obrisi tacku na poziciji `index` (pun Pokrajac deletion).

        Koraci:
          1) odredi RkNN(p) = tacke kojima je p sused -> njima se menja susedstvo;
          2) izbaci p iz njihovih susedstava i iz svih struktura distanci;
          3) reindeksiraj sve (pozicioni sistem) uklanjanjem p;
          4) preracunaj k_dist pogodjenih iz preostalih distanci;
          5) lancano azuriraj lrd i lof pogodjenih i njihovih reverznih suseda.
        """
        if index < 0 or index >= len(self.x_list):
            return self
        if len(self.x_list) == 1:
            # brisemo poslednju tacku -> reset
            self.__init__(n_neighbors=self.n_neighbors,
                          distance_func=self.distance_func)
            return self

        # 1) tacke pogodjene brisanjem: one kojima je `index` bio sused
        affected = set(self.rev_neighborhoods.get(index, []))
        # i reverzni susedi tih (jer lof zavisi od lrd suseda)
        affected_lof = set(affected)
        for a in list(affected):
            affected_lof |= set(self.rev_neighborhoods.get(a, []))
        affected.discard(index)
        affected_lof.discard(index)

        # 2) izbaci `index` iz susedstava pogodjenih (pre reindeksiranja)
        for a in affected:
            if index in self.neighborhoods.get(a, []):
                self.neighborhoods[a] = [j for j in self.neighborhoods[a] if j != index]

        # 3) reindeksiraj sve strukture (ukloni `index`, pomeri ostale)
        (
            self.x_list,
            self.neighborhoods,
            self.rev_neighborhoods,
        ...
        # indeksi pogodjenih posle pomeranja za -1 (oni > index)
        def shift(i):
            return i - 1 if i > index else i
        affected = {shift(a) for a in affected}
        affected_lof = {shift(a) for a in affected_lof}

        k = self.n_neighbors

        # 4) preracunaj k_dist i susedstva pogodjenih.
        # VAZNO: dist_dict je proredjen (cuva samo k najblizih), pa se k_dist
        # NE moze tacno dobiti iz njega posle brisanja. Zato distance pogodjene
        # tacke do SVIH preostalih racunamo ponovo iz x_list (imamo tacke).
        n_pts = len(self.x_list)
        for a in affected:
            # sve distance a -> ostali
            full = {}
            for j in range(n_pts):
                if j == a:
                    continue
                full[j] = self.distance(self.x_list[a], self.x_list[j])
            if not full:
                self.k_dist[a] = float("inf")
                self.neighborhoods[a] = []
                self.dist_dict[a] = {}
                continue
            ordered = sorted(full.values())
            self.k_dist[a] = ordered[min(k, len(ordered)) - 1]
            # zadrzi samo <= k_dist (kao original), to su novi susedi
            kept = {j: d for j, d in full.items() if d <= self.k_dist[a]}
            self.dist_dict[a] = kept
            self.neighborhoods[a] = list(kept.keys())

        # osvezi reverzna susedstva (pogodjeni mogli da promene koga "pokrivaju")
        # bezbedno: rekonstruisi rev iz neighborhoods za sve
        for key in self.rev_neighborhoods:
            self.rev_neighborhoods[key] = []
        for pid, neighs in self.neighborhoods.items():
            for nid in neighs:
                if nid in self.rev_neighborhoods:
                    self.rev_neighborhoods[nid].append(pid)

        # 5a) reach_dist: kad se k_dist pogodjene tacke promeni, menja se
        # reach(i->a) za SVE i kojima je a sused, kao i reach(a->j). Osvezavamo
        # oba smera za pogodjene i njihove reverzne susede.
        recompute_reach = set(affected)
        for a in affected:
            recompute_reach |= set(self.rev_neighborhoods.get(a, []))
        for a in recompute_reach:
            if a not in self.reach_dist:
                self.reach_dist[a] = {}
            for j in self.neighborhoods.get(a, []):
                d = self.dist_dict.get(a, {}).get(j)
                if d is None:
                    d = self.distance(self.x_list[a], self.x_list[j])
                self.reach_dist[a][j] = max(d, self.k_dist.get(j, float("inf")))

        # 5b) lrd svih cija se reach promenila
        upd_lrd = set(recompute_reach)
        self.local_reach = calc_local_reach_dist(
            upd_lrd, self.neighborhoods, self.reach_dist, self.local_reach
        )
        # 5c) lof: svi iz upd_lrd + njihovi reverzni susedi (lof zavisi od lrd suseda)
        upd_lof = set(upd_lrd)
        for a in upd_lrd:
            upd_lof |= set(self.rev_neighborhoods.get(a, []))
        self.lof = calc_lof(
            upd_lof, self.neighborhoods, self.local_reach, self.lof
        )
        return self
